...
function readSessionToken(projectId: string) {
  ...
  return (
    window.sessionStorage.getItem(`relay:project-portal-token:${projectId}`) ??
    ""
  );
}
...
function writeSessionToken(projectId: string, token: string) {
  if (typeof window !== "undefined")
    window.sessionStorage.setItem(
      `relay:project-portal-token:${projectId}`,
      token
    );
}
...
export function draftFromPortal(
  portal: EditorPortal | null
): ProjectPortalDraft {
  return {
...
export function useProjectPortal(projectId: string, enabled: boolean) {
  ...
  const portal = useMemo<EditorPortalWithSessionToken | null>(
    () =>
      result?.portal
  ...
  const save = useCallback(
    async (draft: ProjectPortalDraft) => {
      setError("");
      try {
        const config = {
        ...
        if (!draft.pinProtected && portal?.hasPin)
          await setPin({ portalId, pin: null });
        if (draft.pinProtected && draft.pin.trim())
          await setPin({ portalId, pin: draft.pin.trim() });
      } catch (caught) {
        const message =
  ...
  return useMemo(
    () => ({
      ...
      loading:
        enabled && (authLoading || (isAuthenticated && result === undefined)),
      available,
      error,
      save,
      changeOpen,
