...
5. NAME THE CHEAPEST FIX per driver — index + `.withIndex` instead of scan, `.paginate`/`.take` instead of `.collect`, an aggregate component for counts, caching a hot read — and emit it as a cost-class finding in the local report (evidence: the insight event + the projected growth) pointing at convex-expert/convex-advisor for the actual change.
...
- Every cost driver names its cheapest fix and emits a cost-class finding in the local report pointing at the fixer (convex-expert/advisor).
