...
0. PRECONDITION: preview deployments need a Preview Deploy Key (dashboard → Project Settings → Deploy Keys → Preview) exported as `CONVEX_DEPLOY_KEY` before any `--preview-create`/`--preview-name` deploy — a plain `pnpm exec convex login` session cannot create previews, and this is a paid-tier feature. If no preview key is available, stop before exporting data or starting preview commands and report the missing prerequisite. An isolated development rehearsal needs its own authorized target and procedure; do not run these preview commands against it.
1. GUARD: identify the source and isolated rehearsal target with deploy-guard. Confirm that snapshot access and rehearsal writes are authorized. Rehearsal does not authorize production promotion; prepare its result before requesting any missing promotion approval.
2. SNAPSHOT the source data read-only: `pnpm exec convex export --path snapshot.zip` (from the deployment holding the real data; add `--include-file-storage` only if the migration touches files). This is a read; it changes nothing.
3. CREATE the preview FROM THE PRE-CHANGE CODE — do this BEFORE editing schema.ts, so the preview starts on the schema the snapshot data already conforms to: `pnpm exec convex deploy --preview-create migrate-<slug>` (needs the preview key; auto-expires ~5 days). Seed it: `pnpm exec convex import snapshot.zip --deployment migrate-<slug>` (import targets a deployment by NAME with `--deployment`; there is no `--preview-name` flag on import). The import succeeds because the data still matches the old schema.
4. REHEARSE on the preview, in the migrate order — each push is `pnpm exec convex deploy --preview-name migrate-<slug>` (re-deploys to the SAME preview, keeping its data; NOT `convex dev`, which targets personal dev): (a) make the new/changed field OPTIONAL and deploy — if existing rows violate it the push FAILS HERE on the copy with the offending shape; fix and re-push until green. (b) write a @convex-dev/migrations backfill and run it against the preview; verify every row is now valid. (c) tighten the validator (required / narrowed union) and deploy again — the gate now passes because the backfill ran.
...
6. PROMOTE only when explicitly authorized for the named production target. Present the verified rehearsal result, exact change and recovery plan before requesting any missing final approval. Apply the same verified migration sequence and check the result. A snapshot restore can lose writes made after the snapshot; it is a separate destructive action, not an automatic rollback.
...
- Rehearsal and promotion are separate operations. Honor existing explicit promotion authorization for the same action and target; request missing approval after the rehearsal result is ready.
