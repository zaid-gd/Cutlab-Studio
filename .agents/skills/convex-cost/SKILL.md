...
5. NAME THE CHEAPEST FIX per driver — index + `.withIndex` instead of scan, `.paginate`/`.take` instead of `.collect`, an aggregate component for counts, caching a hot read — and emit it as a cost-class finding in the caller's report using the format defined in [../CONVEX-WORKFLOWS.md](../CONVEX-WORKFLOWS.md) (evidence: the insight event + the projected growth) pointing at convex-expert/convex-advisor for the actual change.
...
- Every cost driver names its cheapest fix and emits a cost-class finding in the caller's report using the format defined in [../CONVEX-WORKFLOWS.md](../CONVEX-WORKFLOWS.md) pointing at the fixer (convex-expert/advisor).
