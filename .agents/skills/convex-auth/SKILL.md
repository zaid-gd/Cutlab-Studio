...
5. Wire the client: ConvexAuthProvider, the sign-in component, and route guards. If you import shadcn/ui primitives (button, input, textarea, label, and so on), reuse the checked-in primitives first. If one is missing, use the repository-pinned shadcn CLI with `pnpm exec shadcn add <name>` after verifying that exact installed version; if none is pinned, select and pin a reviewed version before running it; a missing @/components/ui/* is a hard build error.
...
- Install any missing shadcn/ui primitive up front using the verified repository-pinned CLI from step 5; a missing @/components/ui/* is a hard build failure.
